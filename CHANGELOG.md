...
- Extended primitives, exposed as regular MOGWAI host functions:
  ...
  - `nano.name`, `nano.name.set` — read or set the connected device's name, persisted on the device and reported as the `device` field in `nano.scan`/`nano.select` results. Defaults to `"MogwaiNanoDevice"`; useful to tell multiple devices apart on the same network
