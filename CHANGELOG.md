...
- `console.clear`/`debug.clear` — sends `CONSOLE.CLEAR`/`DEBUG.CLEAR` to a connected Studio, clearing its Console NANO/Debug NANO output remotely from the device's own running program. Currently only understood by the Avalonia Studio (`MogwaiNanoRuntime`'s message dispatcher) — has no effect on the CLI Studio, which doesn't handle these message types
- `SWITCH` — ported from the desktop MOGWAI engine (`switch { (test) then { ... } ... }` sugared on the Studio side, canonical form `{ testBlock1 codeBlock1 ... } SWITCH` on the device). A single block alternating test/code pairs; runs the code block belonging to the first test that returns true, and only that one. Nothing happens if none match — a final `(true) then { ... }` is the common way to write a default/catch-all branch
- `mogwai.exit` — stops the current program immediately, from anywhere, with no error. Complements `mogwai.halt`, which does the same but raises `MW.2` (`HaltEncounteredError`) as a voluntary-halt signal
- **Tasks still running when a program ends are now stopped with a real timeout and a last-resort forced kill**, rather than potentially being left running forever. When a program ends (naturally, or via `mogwai.halt`/`mogwai.exit`/an error), every task it declared is asked to stop cooperatively first (same mechanism as `task.stop`), then the engine waits up to 10 seconds for all of them to actually finish, pumping pending events while it waits. Any task still running after that timeout is forcibly killed via `Thread.Abort()` — genuinely supported on nanoFramework, unlike modern desktop .NET where it's been a non-functional stub since .NET 5 — firing a new `TASK_DID_ABORT` event with the task's name. This is a deliberate last resort: force-killing a task mid-operation can leave whatever hardware peripheral it was using (I2C, GPIO, PWM) in an inconsistent state, so it's meant to be rare, not a routine occurrence
- `==`/`!=` now also work on two `MOGName` values (e.g. `'foo' 'foo' ==`), comparing them by name text — previously raised a `BadArgumentTypeError`
- **Task support — real parallel execution, ported from the desktop engine.** A `task 'name' do { ... }` runs a block of code on its own native `Thread`, with its own fully isolated `MogwaiNanoEngine` instance (own stack, own variables, own everything). Unlike desktop MOGWAI's `Task.Run`-based implementation, NANO tasks are backed directly by `System.Threading.Thread` — nanoFramework has no Task Parallel Library. All communication between a task and its parent happens exclusively through the existing event mechanism (`onEvent`/`event.fire`): `TASK_DID_START`, `TASK_DID_END`, `TASK_DID_FAIL`, `TASK_DID_PUBLISH` (from `task.publish`), and `TASK_DID_RECEIVE` (from the parent's `task.send`) — no other way to share data between a task and its parent, by design. Full primitive set: `TASK.DEF`/`task 'name' do { ... }`, `task.list`, `task.start`, `TASK.START`/`task 'name' start with param`, `task.isRunning`, `task.stop`, `task.purge`, `task.publish`, `task.send`, `task.setResult` (safely copies a value across the thread/instance boundary by serializing to string and re-parsing with the parent's engine, rather than sharing a live object reference), `task.result`, `task.name`, `task.wait`, `task.join` (both pump the parent's pending events while blocking, so `onEvent` handlers keep firing live during the wait), and `mogwai.isTask` (tests whether the currently running program is a task's own engine rather than the top-level program). Only two states exist, matching the desktop engine exactly: `Running` or `Waiting` — the latter covering both "never started" and "finished", with no distinction between the two, so `task.wait`/`task.join` return immediately on an unstarted task rather than erroring.

  **Memory cost, measured:** roughly 7KB for a task's own engine instance plus roughly 3KB for its thread — about 10KB per task. This makes tasks practical only on an ESP32-S3 with PSRAM; on a plain ESP32's ~40KB budget, even a couple of tasks would consume most of the available headroom. See [Memory considerations](../README.md#memory-considerations).

  This was made viable by a large prerequisite refactor: **all 137 registered primitives were converted from instance methods to static methods taking the engine instance as an explicit parameter.** Previously, every primitive was bound to a specific `MogwaiNanoEngine` instance, meaning the entire primitives dictionary had to be rebuilt on every new instance — a real cost multiplied by every concurrent task. The dictionary is now a single static field, built once and shared by every instance regardless of how many tasks are running. Verified after the fact that no piece of per-instance state (open GPIO pins, I2C devices, the execution stack, variable scopes, etc.) was accidentally left `static` in the process — only genuinely constant, safely shareable data (skill names, invalid-character lists) remained static, preserving full task isolation.
...
- `purge` — removes an element, ported from the desktop MOGWAI engine. Behavior depends on what's on the stack: deletes a variable (`'$A' purge`), removes a key from a `MOGRecord` (`[x: 10 y: 20] x: purge` → `[y: 20]`), removes an item from a `MOGList` by index (`(1 2 3 4) 1 purge` → `(1 3 4)`), or removes a byte from a `MOGData` by index (`D:FF5634 1 purge` → `D:FF34`)
- `exists` — tests whether a variable with a given name exists (`'$A' exists` → `.boolean`), ported from the desktop MOGWAI engine. Unlike `purge`, scoped to variables only
...
- **PWM support** — `pwm.open` (name, pin, frequency, duty cycle as a percentage `0`-`100`), `pwm.start`, `pwm.stop`, `pwm.close`. Requires the pin to already be reassigned to a PWM function via `device.setPinFunction` first. Devices are identified by name, following the same pattern as I2C. Validated against real hardware — driving a passive buzzer directly. Changing frequency or duty cycle after opening a channel isn't available yet (close and reopen with new values instead) — deliberately left out for now, partly because some PWM implementations recommend against changing a running channel's frequency. A dedicated `MW.53x` error range covers PWM-specific failures
- **ADC support** — `adc.open` (name, channel), `adc.read`, `adc.close`, `adc.maxValue`, `adc.resolution`. Unlike PWM, ADC channels map to fixed GPIOs and need no `device.setPinFunction` call. `adc.maxValue`/`adc.resolution` expose the controller's actual raw value range and bit resolution at runtime rather than hardcoding a value in the primitive itself, since this can vary by board (e.g. some ESP32-S3 boards default to 13-bit resolution instead of the classic ESP32's 12-bit) — read them once after opening a channel to normalize raw readings correctly regardless of target
- **Units — reusable code libraries stored on the device.** A long-anticipated piece of the puzzle: a *unit* is a named piece of MOGWAI NANO code, permanently stored on the device's flash (`I:\mogwai\units`, no limit on how many), that gets parsed and executed once on demand — typically to declare a set of functions, like a reusable RTC helper library, without having to paste that code into every program that needs it. A unit is a `MOGFunction` under the hood, so it uses the exact same execution machinery as everything else, with no separate caching behavior of its own — it's meant to run once to declare functions, not to be re-run repeatedly as regular program logic. On the Studio side: `nano.units.install` (parses a local `.mog` file and sends its canonical form to the device, named after the source file — pushes `false` if the file can't be found, opened, or parsed, useful for a script that installs whatever units a program needs before running it), `nano.units.purge` (removes a stored unit), `nano.units` (lists stored unit names). On the device side: `mogwai.units` (lists stored units from within a running program), `mogwai.units.run` (executes a stored unit — typically to load its declared functions into the current program's context)
- [ESP32 DeviceFunction values reference](docs/esp32-device-function-values.md) — the complete lookup table needed to use `device.setPinFunction` (SPI, I2C, serial, PWM, ADC, I2S, SDMMC), extracted directly from the `nanoFramework.Hardware.Esp32` package. Now also documents `ESP32_REV3`'s default pin mapping (which buses are already wired without needing `device.setPinFunction` at all) and ADC usage restrictions — most notably, ADC2 channels (10-19) are unavailable whenever WiFi is active, which in practice rules them out entirely for MOGWAI NANO; stick to ADC1 (channels 0-9)
...
- `mogwai.info` (device-side) and `nano.info` (Studio-side) records now also include a `units:` key, listing the names of all units currently stored on the device — the same list queryable via `mogwai.units`/`nano.units`, available from a single info call
...
- `->format`'s `D` (decimal) and `X` (hexadecimal) specifiers only apply to integers in .NET — since `MOGNumber` is `float`-backed, calling `->format` with one of these on a plain number (e.g. `5 "D2" ->format`) silently returned the format string itself (`"D2"`) instead of a formatted result, rather than raising an error. The number is now cast to an integer first when the specifier is `D` or `X`; `F`/`G`/`N` are unaffected and continue to work directly on the float value
