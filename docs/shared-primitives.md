...
| Primitive | Description |
|---|---|
| `->format` | Converts a number to a string using a format mask, e.g. `50 "000" ->format` pushes `"050"`; `50.8 "000.000" ->format` pushes `"050.800"` |
| `->num` | Converts a string to a number when possible; raises an error if the string isn't a valid number |
...
| `FORSTEP` | `start end for 'var' step N do { ... }` |
| `FOREVER` | `forever do { ... }` |
