...
This page only covers what's **common** to both. For NANO-specific primitives (`gpio.*`, `timer.*`, event handling, `nano.*` host functions in MOGWAI NANO Studio), see the [CHANGELOG](../CHANGELOG.md) and the [Getting Started guide](getting-started.md).
...
|---|---|---|
| `MOGNumber` | `.number` | Numeric value. On NANO, backed by `float` rather than `double`, to take advantage of the ESP32's hardware FPU |
...
| `MW.510`–`MW.519` | I2C (NANO only, reserved) |
| `MW.!!!` | Fatal |
